...
bool DataBases::find(string login, string password, User &ptr) {
    if (UsersBase::find(login, password, ptr))
        return true;
    return false;
}

bool DataBases::find(int id, Passenger &ptr) {
    if (PassengersBase::find(id, ptr))
        return true;
    return false;
}

bool DataBases::find(int id, string login, User &ptr) {
    if (UsersBase::find(id, login, ptr))
        return true;
    return false;
}

bool DataBases::find(int routeID, int passID, int status, Ticket &ptr) {
    if (TicketsBase::find(routeID, passID, status, ptr))
        return true;
    return false;
}

bool DataBases::find(int routeID, int passID, Ticket &ptr) {
    if (TicketsBase::find(routeID, passID, ptr))
        return true;
    return false;
}

bool DataBases::find(int fID, QString fDep, QString fDes,
                     QString fMark, int fSeats, Route &ptr) {
    if (RoutesBase::find(fID, fDep, fDes, fMark, fSeats, ptr))
        return true;
    return false;
}

bool DataBases::find(int fID, Route &ptr) {
    if (RoutesBase::find(fID, ptr))
        return true;
    return false;
}

bool DataBases::find(string login, User &ptr) {
    if (UsersBase::find(login, ptr))
        return true;
    return false;
}

bool DataBases::passFound(QString passInfo) {
    if (PassengersBase::find(passInfo.toStdString()))
        return true;
    return false;
}
...
void DataBases::setListPointers(QList<User> *users, QList<Passenger> *passes,
