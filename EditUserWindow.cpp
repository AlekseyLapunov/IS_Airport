            ...
            *userChanged = false;
            ...
            // Если делаем из кассира или администратора пассажира
                ...
                if (otherBox.exec() == QMessageBox::Yes) {
                    ...
                    newPass.exec();
                    if (passTemp) {
                        DBManagerPtr->pushPassenger(pTransm.getFullName(),
                                                    pTransm.getPassport(),
                                                    false, userPtr->getID());
                        *userChanged = true;
                        User transmitter(ui->loginEdit->text().toStdString(),
...
bool EditUserWindow::checkFields()
{
    ...
    if (ui->loginEdit->text().length() < 4
       || ui->loginEdit->text().length() > 12
       || ui->passwordEdit->text().length() < 4
       || ui->passwordEdit->text().length() > 18) {
        mBox.exec();
        return false;
    }
