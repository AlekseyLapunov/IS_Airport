...
void TicketsViewWindow::answerToReq(QModelIndex index) {
    ...
    if (answerBox.exec() == QMessageBox::Yes) {
        if (DBManagerPtr->changeTicket(routeID, passID, statusInt)) {
            if (status == "Покупка" || status == "Возвращён")
                DBManagerPtr->changeRouteSeats(routeID, -1);
            else if (status == "Возврат" || status == "Куплен")
                DBManagerPtr->changeRouteSeats(routeID, 1);
            fillTable();
            gracBox();
        }
...
int TicketsViewWindow::defineStatus(QString ticketStatus) {
    if(ticketStatus == "Покупка")
        return Ticket::stReqAns;
    else if(ticketStatus == "Возврат")
        return Ticket::stRetAns;
    else if (ticketStatus == "Куплен")
        return Ticket::stReq;
    else if (ticketStatus == "Возвращён")
        return Ticket::stRet;
    else return 0;
}
